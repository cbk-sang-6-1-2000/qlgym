    ...
    <div class="row-fluid">
      ...
      <div class='widget-box'>
          <div class='widget-title'> <span class='icon'> <i class='fas fa-th'></i> </span>
            ...
            <div class="container">
              <a href="update.php" class="right-align">Cập nhật</a>
            </div>
          </div>
          ...
          <table class='table table-bordered table-hover'>
             ...
             <?php
                    $qry="select *from khachhang m left JOIN dichvu d ON m.id = d.id where DATEDIFF(m.Ngaykt, CURDATE()) >= 0 AND m.kiemtra = CURDATE()";
           ...
           <tbody>               
                ...
                <td>
                  <div class='text-center'>
                    <a href='kiemtra.php?id=<?php echo $row['user_id'];?>'>
                      <?php if ($row['TT'] == 1): 
                        $sql2 = "SELECT * FROM diemdanh WHERE user_id = '{$row['user_id']}' AND kiemtra = '{$row['kiemtra']}'";
                        $a = $con->query($sql2);
                        $row_exist = mysqli_fetch_array($a);
                      ?>
                        <div class='text-center'><span class="label label-inverse"> <?php echo $row_exist['Ngaydd'];?></span></div>
                        <button class='btn btn-info' disabled style='background-color: red;'>Đã điểm danh <i class='fas fa-map-marker-alt'></i></button>
                      <?php else: ?>
                        <button class='btn btn-info'>Điểm danh <i class='fas fa-map-marker-alt'></i></button>
                      <?php endif; ?>
                    </a>
                  </div>
                </td>         
